...
public class ServerProgram {
	...
	public static void main(String[] args) {
		 ...
		 * - 데이터를 교환하기에 앞서서 서버, 클라이언트가 연결되어있어야함! (서버가 먼저 실행되어 클라이언트의 요청을 기다릴거임)
		 * - 신뢰성 있는 데이터를 전달 가능
		 * 
		 * 소켓(Socket)
		 * - 프로세스간의 통신을 담당
		...
		try {
			...
			// 4) 연결요청이 오면 요청 수락 후 해당 클라이언트와 통신할 수 있는 서버측 소켓 객체 생성
			...
			// 6) 보조스트림을 통해서 성능 개선
			//	  입력용 스트림 => 1바이트짜리 바이트 스트림을 문자스트림으로 변경해줌! => 성능 더 좋은 보조스트림 껴서 
			...
			while(true) {
				...
				// 원래는 finally문에서 close()될 때 자동 호출되는건데 while안에서 정보가 머물러있어서 내보내는 메소드가 필요한 것
			}
