...
public class User {
	 ...
	 *  2. 반환형이 존재하지 않음 (메소드와 구분, 차이 => 만일 반환형을 쓰게 되면 메소드로 인식해버림)
...
//		System.out.println("이거되나???");
		// 기본생성자를 생략했을 경우 => JVM 이 자동으로 만들어줬기 때문에 이때까지 객체생성이 가능했음 => 생략가능
	}
	
	
	// 매개변수 생성자
	public User(String userId, String userPwd, String userName) {
		// 객체 생성과 동시에 전달값들을 매개변수로 받아서 해당 각 필드에 초기화할 목적으로 사용
		this.userId = userId;
	...
	public User(String userId, String userPwd, String userName, int age, char gender) {
		...
		*/
		 ...
		 * this() 생성자 활용 가능
		 * => 같은 클래스 내에 생성자에서 또 다른 생성자를 호출하고자 할 때 사용
