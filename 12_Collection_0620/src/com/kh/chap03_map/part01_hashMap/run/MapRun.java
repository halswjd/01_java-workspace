...
public class MapRun {
	...
	public static void main(String[] args) {
		...
		// 계층 구조를 보면
		// List 계열, Set 계열의 클래스들은 Colletion 구현한 클래스다.
		...
		System.out.println(hm); // {키 = 밸류, 키 = 밸루 ...} 형식으로 출력 
		// 저장되는 순서는 유지 안됨! value 값이 중복돼도 키 값이 중복안되면 잘 저장됨!
		
		hm.put("새우깡", new Snack("매운맛", 700)); 
		// 동일한 키 값으로 다시 추가하는 경우 value값이 덮어씌워짐!! (중복된 키값 공존할 수 없음!! 키 값이 식별자 같은 개념!)
		...
		hm.replace("포테이토칩", new Snack("겁나짠맛", 1000)); // 없는 키값 제시시 무시함
		System.out.println(hm);
		
		// 5. remove(Object key) : 컬렉션에서 해당 키값을 찾아서 그 키-벨류 세트로 삭제시켜주는 메소드
		hm.remove("포테이토칩");
		System.out.println(hm);
